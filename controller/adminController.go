...
func (*Admin) Login(c *fiber.Ctx) error {
	...
	tokenString, errMessage := utils.NewToken().GenJwtToken("admin", usr.ID, 86400)
	if errMessage != "" {
...
func (*Admin) Logout(c *fiber.Ctx) error {
	...
	tokenString, errMessage := utils.NewToken().GenJwtToken("admin", uint(u64), 1)
	if errMessage != "" {
		return c.Status(http.StatusOK).JSON(fiber.Map{
			"message": errMessage,
		})
...
func (*Admin) AdminRefresh(c *fiber.Ctx) error {
	...
	err, accessToken, rfToken := utils.NewToken().AdminRefreshToken(db, rt.Refresh_token, rt.Access_token)
