...
func (*User) UserSignup(c *fiber.Ctx) error {
	...
	fmt.Println("address:" + address.HouseName)
...
func (*User) Verification(c *fiber.Ctx) error {
	...
	fmt.Println("otp:", user.OTP)
	fmt.Println("otp:", user.Name)
	if true {
...
// TokenBlacklist is a simple in-memory blacklist for JWT tokens
...
func (b *TokenBlacklist) AddToken(token string) {
	b.tokens[token] = time.Now()
}

// IsTokenRevoked checks if a token is in the blacklist
func (b *TokenBlacklist) IsTokenRevoked(token string) bool {
	_, ok := b.tokens[token]
	return ok
}
...
func (*User) GenerateInvoice(c *fiber.Ctx) error {
	...
	st, url := utils.UploadPDFToS3[string]("/home/arvinda/Projects/GooCart/media/pdf/"+filen, filen)
	if st {
		fmt.Println("uploaded")
		err := os.Remove("/home/arvinda/Projects/GooCart/media/" + filen)
